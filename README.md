...
#  26.  Cron & Scheduled Tasks
#  27.  Encryption & Hashing
#  28.  Quick Reference
#  29.  Pro Tips & Hidden Gems
#
# ═══════════════════════════════════════════════════════════════════
...
# Screen shortcuts: Ctrl+a c (new), Ctrl+a n (next), Ctrl+a p (prev)
# Ctrl+a d (detach), Ctrl+a k (kill), Ctrl+a ? (help)
...
# Tmux shortcuts: Ctrl+b c (new), Ctrl+b n (next), Ctrl+b p (prev)
...
# Error handling
try { command } catch { Write-Warning "Error: $_" }
try { command } catch [System.Exception] { Write-Host "Error: $_" }
...
Get-PSProvider
New-Item -Path "Data:\file.txt" -ItemType File
...
Get-ChildItem -Recurse -File | Select-Object Name, Length, LastWriteTime, Extension
Get-ChildItem -Recurse -File | Select-Object Name, @{N="SizeMB";E={[math]::Round($_.Length/1MB,2)}}
...
Invoke-Command -ComputerName host -ScriptBlock {Get-Process} -Credential domain\user

New-PSSession -ComputerName host
New-PSSession -ComputerName host -SessionName "Remote"
Get-PSSession
...
Set-Item WSMan:\localhost\Client\TrustedHosts -Value "host"
...
Register-ScheduledTask -TaskName "Backup" -Action (New-ScheduledTaskAction -Execute "cmd.exe" -Argument "/c backup.bat") -Trigger (New-ScheduledTaskTrigger -Daily -At 9am)
Register-ScheduledTask -TaskName "Backup" -Action (New-ScheduledTaskAction -Execute "cmd.exe" -Argument "/c backup.bat") -User "username" -Password "password"
Register-ScheduledTask -TaskName "Backup" -Action (New-ScheduledTaskAction -Execute "cmd.exe" -Argument "/c backup.bat") -User "SYSTEM"
...
Update-ScheduledTask -TaskName "Backup" -Action (New-ScheduledTaskAction -Execute "cmd.exe" -Argument "/c newbackup.bat")
Export-ScheduledTask -TaskName "Backup" | Out-File backup.xml
Import-ScheduledTask -Xml (Get-Content backup.xml) | Register-ScheduledTask -TaskName "Backup"
...
aws s3 cp s3://bucket/file.txt .  # Download from S3
aws s3 sync folder/ s3://bucket/folder/  # Sync to S3
aws s3 sync s3://bucket/folder/ folder/  # Sync from S3
aws s3 mv file.txt s3://bucket/  # Move to S3
aws s3 rm s3://bucket/file.txt  # Delete from S3
aws s3 presign s3://bucket/file.txt  # Generate presigned URL

# EC2
aws ec2 describe-instances  # List EC2 instances
aws ec2 describe-instances --instance-ids id  # Specific instance
aws ec2 start-instances --instance-ids id  # Start EC2 instance
aws ec2 stop-instances --instance-ids id  # Stop EC2 instance
aws ec2 reboot-instances --instance-ids id  # Reboot EC2
aws ec2 create-tags --resources id --tags Key=Name,Value=name  # Tag instance
aws ec2 describe-security-groups  # List security groups
aws ec2 create-security-group --group-name name --description desc  # Create SG

# RDS
aws rds describe-db-instances  # List RDS instances
aws rds start-db-instance --db-instance-identifier id  # Start RDS
aws rds stop-db-instance --db-instance-identifier id  # Stop RDS

# Lambda
aws lambda list-functions  # List Lambda functions
aws lambda invoke --function-name name output.json  # Invoke Lambda
...
az vm show --name vmname --resource-group rg  # Show VM details
az vm start --name vmname --resource-group rg  # Start VM
az vm stop --name vmname --resource-group rg  # Stop VM
az vm restart --name vmname --resource-group rg  # Restart VM
az vm create --name vmname --resource-group rg --image UbuntuLTS --admin-username user --admin-password pass  # Create VM
az vm delete --name vmname --resource-group rg --yes  # Delete VM

# Storage
az storage account list  # List storage accounts
az storage account show --name name  # Show storage account
az storage account keys list --name name  # List account keys
az storage blob upload --file file.txt --container container --name blob  # Upload blob
az storage blob download --container container --name blob --file file.txt  # Download blob
...
az functionapp show --name name --resource-group rg  # Show function app

# ─── GCP CLI ──────────────────────────────────────────────────────

gcloud init               # Initialize GCP
...
gcloud compute instances list --project project-id  # In specific project
gcloud compute instances start instance-name  # Start VM
gcloud compute instances stop instance-name  # Stop VM
gcloud compute instances reset instance-name  # Reset VM
gcloud compute instances create instance-name --zone zone --machine-type n1-standard-1 --image-family ubuntu-2004-lts --image-project ubuntu-os-cloud  # Create VM
gcloud compute instances delete instance-name  # Delete VM

# Storage
gcloud storage buckets list  # List buckets
gcloud storage buckets list --project project-id  # In specific project
gcloud storage cp file.txt gs://bucket/  # Upload to bucket
gcloud storage cp gs://bucket/file.txt .  # Download from bucket
gcloud storage ls gs://bucket  # List bucket contents
gcloud storage rm gs://bucket/file.txt  # Delete from bucket
...
# Git, Databases, Cloud CLI, and much more.
#
#
# ---
# Author: F1REW0LF
